...
class AssetAssignmentService
{
    ...
    /**
     * Mengassign aset ke pemegang baru.
     */
    public function assign(
        Asset $asset,
        AssignAssetData $data,
        User $actor,
    ): Asset {
        return DB::transaction(function () use ($asset, $data, $actor): Asset {
            ...
            if ($locked->activeAssignment()->exists()) {
                throw new StateConflictException(
                    'Aset masih memiliki pemegang aktif.',
                );
            }
            ...
            if (! $user) {
                throw ValidationException::withMessages([
                    'user_id' => [
                        'Pegawai yang dipilih tidak valid atau tidak aktif',
                    ],
                ]);
            }
    ...
    /**
     * Melepaskan aset dari pemegang aktif.
     */
    public function release(
        Asset $asset,
        ReleaseAssetData $data,
        User $actor,
    ): Asset {
        return DB::transaction(function () use ($asset, $data, $actor): Asset {
            /** @var Asset $locked */
            ...
            if (! $assignment) {
                throw new StateConflictException(
                    'Aset tidak memiliki pemegang aktif.',
                );
            }
    ...
    /**
     * Helper untuk mencatat riwayat pemegangan aset.
     */
