...
test("employee hanya bisa melihat aset yang dia miliki", function () {
    $employee = User::factory()->employee()->create();
    $otherEmployee = User::factory()->employee()->create();

    $ownAsset = Asset::factory()
        ->assigned()
        ->create(["name" => "Own Laptop"]);
    ...
    $pastAsset = Asset::factory()
        ->available()
        ->create(["name" => "Past Asset"]);
    AssetAssignment::factory()->create([
...
test("my-assets endpoint support pagination", function () {
    $employee = User::factory()->employee()->create();
    Sanctum::actingAs($employee);
    ...
    Asset::factory()
        ->count(5)
        ->create()
        ->each(function ($asset) use ($employee) {
            AssetAssignment::factory()->create([
                "asset_id" => $asset->id,
                "user_id" => $employee->id,
                "released_at" => null,
