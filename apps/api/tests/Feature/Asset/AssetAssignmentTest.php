...
test('tidak bisa assign asset maintenance', function () {
...
test('tidak bisa assign asset retired or lost', function () {
    $retired = Asset::factory()->retired()->create();
    $lost = Asset::factory()->lost()->create();
    $employee = User::factory()->employee()->create();
    Sanctum::actingAs(User::factory()->manager()->create());

    $this->postJson("/api/assets/{$retired->id}/assign", [
        'user_id' => $employee->id,
    ])
        ->assertStatus(422)
        ->assertJsonValidationErrors('status');

    $this->postJson("/api/assets/{$lost->id}/assign", [
        'user_id' => $employee->id,
    ])
        ->assertStatus(422)
        ->assertJsonValidationErrors('status');
...
test('tidak bisa assign asset dengan assignment aktif', function () {
...
test(
    'tidak bisa assign asset ke user yang tidak ada atau tidak aktif',
    function () {
...
test('assign asset berhasil dan mencatat history dan audit', function () {
    ...
    expect(
        AssetHistory::where('asset_id', $asset->id)
            ->where('action', AssetHistoryAction::Assigned->value)
            ->count(),
    )->toBe(1);
    expect(
...
test('release asset berhasil dan mencatat history dan audit', function () {
    $asset = Asset::factory()->assigned()->create();
    $employee = User::factory()->employee()->create();
    ...
    expect(
        AssetHistory::where('asset_id', $asset->id)
            ->where('action', AssetHistoryAction::Released->value)
            ->count(),
    )->toBe(1);
    expect(
        AuditLog::where('module_id', $asset->id)
...
test('tidak bisa release asset tanpa assignment aktif', function () {
    $asset = Asset::factory()->available()->create();
    Sanctum::actingAs(User::factory()->manager()->create());
