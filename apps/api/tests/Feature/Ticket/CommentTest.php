...
test(
    "partisipan bisa menambahkan komentar pada tiket yang terbuka dan menerima 201",
    function () {
...
test(
    "employee non partisipan tidak dapat menambahkan komentar dan menerima 404",
    function () {
        $reporter = User::factory()->employee()->create();
        ...
        $this->postJson("/api/tickets/{$ticket->id}/comments", [
            "body" => "Komentar nyasar",
        ])->assertStatus(404);
    },
);

test(
    "technician yang tidak ditugaskan tidak dapat menambahkan komentar dan menerima 404",
    function () {
        $assignedTech = User::factory()->technician()->create();
        $otherTech = User::factory()->technician()->create();
        $ticket = Ticket::factory()
            ->inProgress()
            ->create(["technician_id" => $assignedTech->id]);
...
test("komentar pada tiket yang sudah ditutup ditolak dengan 403", function () {
    $reporter = User::factory()->employee()->create();
    $ticket = Ticket::factory()
        ->closed()
        ->create(["reporter_id" => $reporter->id]);
    ...
    Sanctum::actingAs($reporter);
    $this->postJson("/api/tickets/{$ticket->id}/comments", [
        "body" => "Tiket sudah ditutup",
    ])->assertStatus(403);
});

test("validasi error mengembalikan 422 dengan pesan Indonesia", function () {
    $reporter = User::factory()->employee()->create();
    $ticket = Ticket::factory()
        ->open()
        ->create(["reporter_id" => $reporter->id]);
...
test(
    "komentar pada tiket yang terbuka ditampilkan dengan urutan ascending berdasarkan waktu pembuatan",
    function () {
        $reporter = User::factory()->employee()->create();
        $ticket = Ticket::factory()
            ->open()
...
test(
    "notif komentar diterima oleh partisipan yang terdaftar tapi tidak oleh pembuat",
    function () {
        $reporter = User::factory()->employee()->create();
...
test("pembuat dapat memperbarui komentar dalam 15 menit", function () {
    $reporter = User::factory()->employee()->create();
    $ticket = Ticket::factory()
        ->open()
        ->create(["reporter_id" => $reporter->id]);
...
test("pembuat tidak dapat memperbarui komentar setelah 15 menit", function () {
    $reporter = User::factory()->employee()->create();
    $ticket = Ticket::factory()
        ->open()
        ->create(["reporter_id" => $reporter->id]);
...
test(
    "admin dapat memperbarui komentar apapun tanpa batasan dalam 15 menit",
    function () {
...
test(
    "pembuat dapat menghapus komentar dalam 15 menit dan komentar tersebut dihapus secara soft",
    function () {
        $reporter = User::factory()->employee()->create();
        $ticket = Ticket::factory()
            ->open()
...
test(
    "rute komentar mengembalikan 404 jika komentar tidak termasuk dalam tiket",
    function () {
        $reporter = User::factory()->employee()->create();
        ...
        Sanctum::actingAs($reporter);
        $this->putJson("/api/tickets/{$ticket2->id}/comments/{$comment->id}", [
            "body" => "Mismatch",
        ])->assertStatus(404);
        $this->deleteJson(
            "/api/tickets/{$ticket2->id}/comments/{$comment->id}",
        )->assertStatus(404);
    },
);
