...
test("golden path selesai end to end melalui HTTP", function () {
    // 1. Actors
    $employee = User::factory()->employee()->create();
    $manager = User::factory()->manager()->create();
    $technician = User::factory()->technician()->create();

    // 2. Employee membuat tiket baru
    ...
    // 3. Manager meng-assign technician
    ...
    // Verifikasi notifikasi telah diterima oleh technician
    $this->assertDatabaseHas("notifications", [
        "user_id" => $technician->id,
        "type" => NotificationType::TicketAssigned->value,
    ]);

    // 4. Technician mulai bekerja (200 OK -> IN_PROGRESS)
    ...
    // 5. Technician menambahkan komentar (201 Created)
    $this->postJson("/api/tickets/{$ticketId}/comments", [
        ...
        ->assertJsonPath(
            "data.body",
            "Sedang dicek kabel HDMI dan roll back driver GPU.",
        );

    // Verifikasi notifikasi telah diterima oleh employee
    $this->assertDatabaseHas("notifications", [
        "user_id" => $employee->id,
        "type" => NotificationType::TicketCommented->value,
    ]);

    // 6. Technician selesai ticket (200 OK -> RESOLVED)
    $this->postJson("/api/tickets/{$ticketId}/status", [
    ...
    // 7. Employee konfirmasi dan menutup ticket (200 OK -> CLOSED)
    Sanctum::actingAs($employee);
    $this->postJson("/api/tickets/{$ticketId}/status", [
        "status_id" => 5, // CLOSED
    ...
    // 8. Verify final detail endpoint
    ...
    // 9. Verifikasi timeline history berisi progres lengkap
    $historyResponse = $this->getJson(
        "/api/tickets/{$ticketId}/histories",
    )->assertStatus(200);

    $histories = $historyResponse->json("data");
    expect(count($histories))->toBeGreaterThanOrEqual(5);

    // Verifikasi nilai history dapat dibaca sebagai label
    $statusHistories = array_filter(
        $histories,
        fn($h) => $h["field_changed"] === "status_id",
    );
    $statusTransitions = array_map(
        fn($h) => [$h["old_value"], $h["new_value"]],
        array_values($statusHistories),
    );
